...
The status row above the function keys labels completion, cancellation, and failure without interrupting browsing. Copy, Move, and Delete confirmations retain their originally captured files while listings refresh.
...
- Markdown inputs up to 1 MiB (`.md`, `.markdown`, `.mdown`, `.mkd`, case-insensitive) renders headings, emphasis, lists, task lists, quotes, tables, links, and highlighted fenced code. Prose wraps on resize; code blocks keep their layout. Image references show their descriptions and paths. Links and embedded HTML are inert; viewing never fetches referenced content.
  Larger Markdown inputs remain fully browsable as plain text with line numbers and a persistent styling-limit notice. The limit bounds parser input, not total memory; the plain-text index is reused across resizes.
