...
2026-09-09 — type-to-jump pane navigation:
- Extended the existing quick search so ordinary typing immediately jumps to a case-insensitive filename prefix in the active pane. Used the proposed defaults: literal prefix matching and a 1.5-second inactivity reset. Directories and all VFS listings use their existing names; no extra filesystem reads. Incoming listing batches reapply an active query.
- Show the query and no-match feedback in the existing pane bottom border without adding rows. No match preserves the cursor; Backspace edits Unicode text. Escape cancels without treating the next character as Alt; normal navigation, mouse actions, selection, and operation shortcuts dismiss the query and retain their actions. Idle automatic refresh pauses while searching.
- Preserved Ctrl+S/Alt+S persistent wildcard quick search and next-match cycling with wraparound, including explicit entry of spaces and selection-key characters. Direct typing reserves Space, +, -, *, and backslash for existing MC selection shortcuts. Updated keyboard help and README.
- Validation passed: all 45 standard Rust tests, including four new keyboard/rendering regressions; formatting; strict Clippy; debug build; existing terminal smoke suite extended to exercise type-to-jump. Tests cover case/Unicode, directories, literal punctuation, no matches, cycling, timeout, Backspace, panel switching, dialogs, selection, Esc+digit shortcuts, scrolling, and border feedback. Changes remain uncommitted.

2026-09-09 — F2 rename in place:
- Added F2, its shortcut-bar button, and a File menu action to rename only the highlighted item. The dialog captures the source path and starts with its current name. Enter confirms; Escape cancels; Ctrl+U clears. Reject empty names, paths, control characters, . and ..; unchanged names are a no-op. Parent rows and read-only archive entries cannot be renamed.
- Added an exact-target, single-source Rename job with source/destination locks. Existing names fail without merging or overwriting. Successful completion refreshes panels, follows the renamed item, and transfers any selection on that item to its new path. Errors appear in a Rename message and remain in Background jobs.
- Added explicit VFS rename dispatch with no copy/delete fallback after failure. Local/SFTP reuse no-replace rename; FTP checks then issues RNFR/RNTO; SSH uses atomic no-replace native primitives through the fixed helper on Linux/macOS and rejects unsupported platforms. FTP retains its documented concurrent-client race. Uncertain remote replies require inspecting both names before manual retry.
- Corrected Windows local no-replace rename to use MoveFileExW without replacement flags instead of std::fs::rename, with extended-length paths and NUL checks. Added a Windows-only windows-sys dependency already present in the lockfile. Reference: https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-movefileexw . Windows/macOS native execution is pending CI.
- Validation passed: 48 standard Rust tests; strict Clippy; formatting; debug build; local terminal smoke with F2 rename and cursor-follow; FTP/SFTP/SSH rename contracts for Unicode/quoted filenames, directories, and existing-target refusal; remote and job terminal suites; Python syntax and diff checks. Updated the terminal fixture's empty-job assertion because rename now creates completed jobs. README/VFS document behavior and limits. Changes remain uncommitted, alongside the preceding type-to-jump feature.

2026-09-09 — browse JAR and ZIP-based Office files:
